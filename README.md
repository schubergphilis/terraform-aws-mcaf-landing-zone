...
| <a name="input_regions"></a> [regions](#input\_regions) | Region configuration, plus global and per-region service SCP exceptions. See the README for more information on the configuration options. | <pre>object({<br/>    additional_allowed_service_actions_per_region = optional(map(list(string)), {})<br/>    allowed_regions                               = list(string)<br/>    home_region                                   = string<br/>    linked_regions                                = optional(list(string), ["us-east-1"])<br/>    enable_securitylake_aggregation_actions       = optional(bool, false)<br/>  })</pre> | n/a | yes |
